...
# generate data2id dic
def generate_data_idx_for_mc(file_path, entity2id, property2id):
    data2id = []
    with open(file_path, "r", encoding="utf-8") as f:
        for line in f:
            if len(line.strip()) > 1:
                line_parts = line.strip().split()
                data2id.append([entity2id[line_parts[0]], entity2id[line_parts[1]], property2id[line_parts[2]], int(line_parts[-1])])
    return data2id
...
# get (S,S')
def get_tuples(Sbatch, sample_ent_cand_ids, sample_attr_cand_ids, conid2Attrid, conAttr2id_set, train2id_set, ca_flag=False):
    Tbatch = []
...
def get_con_attrs(train2id, ca2id):
    ent_attrs = {}
    ent_fre = {}
    for tri in train2id:
        t,s,a = tri[0], tri[1], tri[2]
        if t not in ent_attrs.keys():
            ent_fre[t] = 1
            ent_attrs[t] = {a: 1}
        else:
            ent_fre[t] += 1
    ...
    for caf in ca2id:
        c,a, fre = caf[0], caf[1], caf[2]
        if c not in ent_attrs.keys():
            ent_fre[c] = fre
            ent_attrs[c] = {a: fre}
        else:
            ent_fre[c] += fre
    ...
    return ent_attrs, ent_fre


# link prediction for head, tail ,property
def link_prediction_with_cilin_tag(test2id, train2id, ca2id, model, id2entity, id2property, entity_candidate_ids, property_candidate_ids, entid2tags, proid2tags, flag, use_cuda):
    mean_rank = []
    hits_10_counts = 0
    mean_rank_tag = []
    ...
    print("The Hits 10%% is: %f%%" % ((hits_10_counts / len(test2id)) * 100))
    print("The mean rank with tag is: ", sum(mean_rank_tag) / len(mean_rank_tag))
    print("The Hits 10%% with tag is: %f%%" % ((hits_10_counts_tag / len(test2id)) * 100))
...
def triplet_classification(triplets, model, threshold):
    score_list = []
    for t in triplets:
        t = t.view(-1, 1)
        score = model.predict(t[0], t[1], t[2])
